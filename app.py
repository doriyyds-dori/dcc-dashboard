...
def process_data(path_f, path_d, path_a, path_s):
    try:
        ...
        # ================= D. AMS (跟进数据) - 改进版 =================
        # ✅ 直接用 AMS 表的真实列名进行映射，优先保留原表的"率"字段
        ...
        # 5️⃣ 计算"率"（仅当原表【没有】该字段时才自算，避免覆盖源表数据）
        ...
        # 6️⃣ 确保所有"率"列都是 0~1 的小数形式（不是百分比）
